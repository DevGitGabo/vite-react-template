...
const GenPePor = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        // Filtrar los registros por el status "PENDIENTE"
        const pendientes = res.data.filter(
          (registro) => registro.status === "PENDIENTE"
        );
    ...
    >
      ...
      <div className="flex flex-wrap gap-4 max-h-[600px] overflow-y-scroll scrollbar-thin">
        {records.map((d, i) => (
          ...
          <div
            key={i}
            className="bg-[#F8EEE6] w-fit h-fit rounded-lg  overflow-hidden"
          >
            <div>
              {console.log(records)} 
              <img
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      <span className="font-semibold">
                        Total a Pagar S/.{selectedPedido.monto_total}
                      </span>
                    </div>
                  </div>
                  <div className="mt-4">
                    <label
                      htmlFor="status"
                      className="mr-1 font-medium text-lg"
                    >
                      Estado:
                    </label>
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        // Aquí puedes agregar la lógica para enviar el status actualizado a tu API
                        axios
                          .put(
                            `https://backendcasona-production.up.railway.app/cashier/update/${selectedPedido.id}`,
                            { status: newStatus }
                          )
                          .then((res) => {
